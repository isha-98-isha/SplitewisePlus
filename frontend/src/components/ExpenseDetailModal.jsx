...
function ExpenseDetailModal({ expense, settlements, isOpen, onClose, userData }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 min-h-screen">
            ...
            {isImageFullscreen && expense.receipt && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center bg-stone-900/95 backdrop-blur-md p-4 animate-in zoom-in duration-200">
                    <button
                        onClick={() => setIsImageFullscreen(false)}
                        className="absolute top-6 right-6 p-3 bg-white/10 text-white hover:bg-white/20 rounded-full transition-colors backdrop-blur-sm"
                    >
                        <X size={24} />
                    </button>
                    <img src={expense.receipt} alt="Receipt proof fullscreen" className="max-w-full max-h-full object-contain rounded-lg" />
                </div>
            )}

            {/* Modal Content */}
            <div className="relative bg-white w-full max-w-lg max-h-[90vh] flex flex-col rounded-[2.5rem] shadow-2xl shadow-stone-900/20 overflow-hidden animate-in zoom-in slide-in-from-bottom-4 duration-300 border border-stone-100">
                <div className="h-2 bg-amber-800 w-full shrink-0" />

                {/* Header */}
                <div className="p-6 md:p-8 pb-4 flex items-start justify-between shrink-0">
                ...
                <div className="overflow-y-auto custom-scrollbar flex-1">
                    {/* Amount Section */}
                    <div className={`px-6 md:px-8 py-4 flex gap-4 ${expense.totalSplit <= 1 ? "justify-center" : ""}`}>
                    ...
                    <div className="p-6 md:p-8 pt-2 space-y-6">
                        ...
                        {/* Receipt Image */}
                        {expense.receipt && (
                            <div className="bg-stone-50 p-4 rounded-2xl border border-stone-100 group cursor-pointer" onClick={() => setIsImageFullscreen(true)}>
                                <div className="flex items-center justify-between mb-3">
                                    <p className="text-[9px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-2">
                                        Receipt / Proof
                                    </p>
                                    <p className="text-[8px] font-bold text-amber-800 opacity-0 group-hover:opacity-100 transition-opacity uppercase tracking-widest bg-amber-100 px-2 py-0.5 rounded-md">
                                        Click to enlarge
                                    </p>
                                </div>
                                <div className="rounded-xl overflow-hidden border border-stone-200">
                                    <img src={expense.receipt} alt="Receipt proof" className="w-full h-auto object-contain max-h-48 group-hover:scale-105 transition-transform duration-300" />
                                </div>
                            </div>
                        )}
                ...
                {/* Footer Action */}
                <div className="bg-stone-50 p-6 shrink-0 flex justify-center border-t border-stone-100">
                    <button
                        onClick={onClose}
